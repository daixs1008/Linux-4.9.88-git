			 ...
			 const char *devname, void *dev_id)
{
	...
	if (!irq_settings_can_request(desc) ||    //并非系统中所有的IRQ number都可以request，有些中断描述符被标记为IRQ_NOREQUEST，标识该IRQ number不能被其他的驱动request。
	    WARN_ON(irq_settings_is_per_cpu_devid(desc)))     //irq_settings_is_per_cpu_devid函数用来判断一个中断描述符是否需要传递per cpu的device ID。
		return -EINVAL;
