 ...
 * 在结构体选项中验证选项和填充指针。调用者应该清除*opt，并设置opt->data。
 * 如果opt == NULL，那么skb->data应该指向IP报头。
 */
// 会在两种不同的情况下调用
// 1. ip_rcv_finish调用以解析和验证输入封包的IP选项
// 2. 由ip_options_get 调用
		       ...
		       struct ip_options *opt, struct sk_buff *skb)
{
	...
	if (skb) {  // 入口封包时，skb不为null， 封包正被传输（发送）时 skb为 null
		rt = skb_rtable(skb);
