...
static void __init cacheid_init(void)
{
	...
	pr_info("CPU: %s data cache, %s instruction cache\n",    //printf-5--CPU: PIPT / VIPT nonaliasing data cache, VIPT aliasing instruction cache
		cache_is_vivt() ? "VIVT" :
...
static void __init patch_aeabi_idiv(void)
{
	...
	pr_info("CPU: div instructions available: patching division code\n");  //printf-4--CPU: div instructions available: patching division code
...
void notrace cpu_init(void)
{
	...
	/*
	 * This only works on resume and secondary cores. For booting on the  这只适用于简历和次要核心内容。 为了在引导cpu上引导，
	 * boot cpu, smp_prepare_boot_cpu is called after percpu area setup.  在percpu区域设置之后调用smp_prepare_boot_cpu。  
	 */
	 ...
	 * In Thumb-2, msr with an immediate value is not allowed.  定义下面的内联asm指令的位置约束。 在Thumb-2中，不允许msr带有直接值。  
	 */
...
#define PLC	"I"
	...
	/*
	 * setup stacks for re-entrant exception handlers 为可重入异常处理程序设置堆栈  
	 */
	__asm__ (
	"msr	cpsr_c, %1\n\t"      //让CPU进入IRQ mode
	"add	r14, %0, %2\n\t"   //r14寄存器保存stk->irq
...
void __init smp_setup_processor_id(void)
{
	...
	pr_info("Booting Linux on physical CPU 0x%x\n", mpidr);  //printf-1--Booting Linux on physical CPU 0x0
}
...
{
	struct proc_info_list *list;//proc_info_list CPU 的对象列表
	...
	pr_info("CPU: %s [%08x] revision %d (ARMv%s), cr=%08lx\n",   //3--CPU: ARMv7 Processor [410fc075] revision 5 (ARMv7), cr=10c53c7d
	...
	cacheid_init();  //CPU: PIPT / VIPT nonaliasing data cache, VIPT aliasing instruction cache
	cpu_init();  //初始化一个CPU
}
...
void __init setup_arch(char **cmdline_p)
{
	...
	if (!mdesc)
		mdesc = setup_machine_tags(__atags_pointer, __machine_arch_type); //如果没有设备时走这个分支  已经有设备树就不走这个分支了
	machine_desc = mdesc;
	...
	*cmdline_p = cmd_line;
	...
	early_fixmap_init(); //主要是建立 early PGD/PUD/PMD页表。
	early_ioremap_init(); //根据上面建立的早期的页表 创建内存映射

	parse_early_param(); //早期参数和命令行 解析

#ifdef CONFIG_MMU
	early_paging_init(mdesc);// 早期页表初始化
#endif
	setup_dma_zone(mdesc);  //设置DMA 的区域
	xen_early_init();
	efi_init();
	/*
	 ...
	 */
	adjust_lowmem_bounds();  //调整低内存的边界
	arm_memblock_init(mdesc); //内存块初始化
	/* Memory may have been removed so recalculate the bounds. */
	adjust_lowmem_bounds();  //重新调整内存边界

	early_ioremap_reset();  //早期IO 映射重新设置

	paging_init(mdesc);  //页表初始化
	request_standard_resources(mdesc); //标准的资源请求

	if (mdesc->restart)
		arm_pm_restart = mdesc->restart;  //根据设备时更新物理地址
