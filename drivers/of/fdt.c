 ...
 *
 * It returns the size of unflattened device tree or error code   //返回设备树的大小或者错误代码
 */
...
#define FDT_MAX_DEPTH	64
	...
	for (offset = 0;
	     ...
	     offset = fdt_next_node(blob, offset, &depth)) {  //使用for 循环解析每个设备树节点
...
static void *__unflatten_device_tree(const void *blob,
				     struct device_node *dad,
				     struct device_node **mynodes,   //指针的指针是需要返回数据的
...
{
	return of_fdt_match(initial_boot_params, node, compat);  // fdt:0x8137dd7c
}
		...
		const void * (*get_next_compat)(const char * const**))
{
	...
	dt_root = of_get_flat_dt_root();  //得到设备树根节点
	while ((data = get_next_compat(&compat))) {
		score = of_flat_dt_match(dt_root, compat); //匹配设备树   score  设备树匹配的成绩  最小的成绩对应的设备就是要找的设备
...
bool __init early_init_dt_verify(void *params)
{
	...
	/* check device tree validity */ //检查设备树是否存在
