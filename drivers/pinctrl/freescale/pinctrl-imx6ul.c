...
static int imx6ul_pinctrl_probe(struct platform_device *pdev)
{
	...
	match = of_match_device(imx6ul_pinctrl_of_match, &pdev->dev);//内部实现：__of_device_is_compatible(node, matches->compatible, matches->type, matches->name);
					
	if (!match)
		return -ENODEV;   //如果没有匹配项，退出返回

	pinctrl_info = (struct imx_pinctrl_soc_info *) match->data;

	return imx_pinctrl_probe(pdev, pinctrl_info);//-52--///imx_pinctrl的通用函数
}
