...
static const struct pinctrl_ops imx_pctrl_ops = {
	.get_groups_count = imx_get_groups_count,  //获取有多少组引脚
	.get_group_name = imx_get_group_name,  //获取每一组引脚的名字
	.get_group_pins = imx_get_group_pins,//获得每一组当中的众多引脚
	.pin_dbg_show = imx_pin_dbg_show,
	.dt_node_to_map = imx_dt_node_to_map,//将设备树节点（device tree node）解析出一系列的结构体（pinctrl_map-->pinctrl-setting）存入pinctrl-state
...
static const struct pinmux_ops imx_pmx_ops = {
	.get_functions_count = imx_pmx_get_funcs_count,  //得到设备树支持多少个功能
	.get_function_name = imx_pmx_get_func_name,  //获得功能的名字
	.get_function_groups = imx_pmx_get_groups,  //在这个功能下面有多少组引脚
	.set_mux = imx_pmx_set,                        //把某一组引脚配置为某一个功能
	.gpio_set_direction = imx_pmx_gpio_set_direction,  //设置引脚的方向
};
...
static const struct pinconf_ops imx_pinconf_ops = {
	.pin_config_get = imx_pinconf_get,  //获得当前配置
	.pin_config_set = imx_pinconf_set,  //设置当前配置
				       ...
				       u32 index)
{
	...
	for_each_child_of_node(np, child) {
		...
		imx_pinctrl_parse_groups(child, grp, info, i++); //-52--
	}
		      ...
		      struct imx_pinctrl_soc_info *info)
{
	...
	struct pinctrl_desc *imx_pinctrl_desc;   //主要过程：分配，设置，注册  这个结构体
	struct device_node *np;
	...
	imx_pinctrl_desc = devm_kzalloc(&pdev->dev, sizeof(*imx_pinctrl_desc), //使用devm_kzalloc 分配结构体  	在remove 函数中不需要释放了，
					GFP_KERNEL);
	if (!imx_pinctrl_desc)
		return -ENOMEM;

	imx_pinctrl_desc->name = dev_name(&pdev->dev);     //设置
	imx_pinctrl_desc->pins = info->pins;  //得到单个引脚
	imx_pinctrl_desc->npins = info->npins;
	imx_pinctrl_desc->pctlops = &imx_pctrl_ops;//解析设备树，得到一组引脚
	imx_pinctrl_desc->pmxops = &imx_pmx_ops;  //把某一组引脚配置为某一个功能
	imx_pinctrl_desc->confops = &imx_pinconf_ops;//把某一组或某一个引脚配置成某个功能
	imx_pinctrl_desc->owner = THIS_MODULE;

	ret = imx_pinctrl_probe_dt(pdev, info); //-51--
	if (ret) {
	...
	ipctl->pctl = devm_pinctrl_register(&pdev->dev,        //注册--把imx_pinctrl_desc 添加到device  		中
	...
	dev_info(&pdev->dev, "initialized IMX pinctrl driver\n"); //printf-52--imx6ul-pinctrl 20e0000.iomuxc: initialized IMX pinctrl driver
															//printf-53--imx6ul-pinctrl 2290000.iomuxc-snvs: initialized IMX pinctrl driver
	return 0;
}
