...
{
	wait_event_timeout(i2c_imx->queue, i2c_imx->i2csr & I2SR_IIF, HZ / 10);  // 等待事件，并且超时退出
...
static int i2c_imx_read(struct imx_i2c_struct *i2c_imx, struct i2c_msg *msgs, bool is_lastmsg)
{
	...
	result = i2c_imx_trx_complete(i2c_imx);  // 等待完成，有中断会唤醒，超时没有中断就会超时返回
	if (result)
		return result;
	result = i2c_imx_acked(i2c_imx);
	if (result)
	...
	if ((msgs->len - 1) || block_data)
		temp &= ~I2CR_TXAK;
	imx_i2c_write_reg(temp, i2c_imx, IMX_I2C_I2CR);
	imx_i2c_read_reg(i2c_imx, IMX_I2C_I2DR); /* dummy read 虚的读，此时读的数据无效，为了启动传输，芯片要求这样做*/
	...
	for (i = 0; i < msgs->len; i++) {
		...
		if ((!i) && block_data)
			...
			msgs->buf[i] = imx_i2c_read_reg(i2c_imx, IMX_I2C_I2DR);  // 读数据保存在buff里面
		dev_dbg(&i2c_imx->adapter.dev,
						...
						struct i2c_msg *msgs, int num)
{
	...
	/* Start I2C transfer */
	result = i2c_imx_start(i2c_imx);  // 启动传输
	if (result) {
