...
static int i2c_device_probe(struct device *dev)
{
	...
	if (!client->irq) {
		...
		if (dev->of_node) {
			irq = of_irq_get_byname(dev->of_node, "irq");  //»ñÈ¡ÖÐ¶Ï×ÊÔ´
...
static int i2c_register_adapter(struct i2c_adapter *adap)
{
	...
	res = device_register(&adap->dev);   //×¢²áÉè±¸ adapter  ´Ëº¯Êý·µ»Ø 0 ±íÊ¾adapter ×¢²á³É¹¦
	if (res) {
		pr_err("adapter '%s': can't register device (%d)\n", adap->name, res);
		goto out_list;
	}

	dev_dbg(&adap->dev, "adapter [%s] registered\n", adap->name);  //µ½´ËÒ»¸öI2C adapter ×¢²á³É¹¦
...
int i2c_add_adapter(struct i2c_adapter *adapter)
{
	...
	if (dev->of_node) {    //Èç¹ûÓÐÉè±¸Ê÷½Úµã ¾Í¸ù¾ÝÉè±¸Ê÷½ÚµãÐÅÏ¢»ñÈ¡ adap->nr È·¶¨I2C ¿ØÖÆÆ÷ºÅ
		id = of_alias_get_id(dev->of_node, "i2c");
		if (id >= 0) {
			adapter->nr = id;
			return __i2c_add_numbered_adapter(adapter);  //¸ù¾Ý adap->nr ×¢²áÖ¸¶¨ I2C ¿ØÖÆÆ÷
		} 
	}
...
struct i2c_adapter *i2c_get_adapter(int nr)
{
	...
	mutex_lock(&core_lock);
	adapter = idr_find(&i2c_adapter_idr, nr);  // ä½¿ç”¨æ¬¡è®¾å¤‡å·æ‰¾åˆ° æŽ§åˆ¶å™¨ (i2c_adapter)  æ‰€æœ‰çš„i2c_adapteréƒ½ä¼šæ”¾åœ¨i2c_adapter_idré“¾è¡¨é‡Œ
	if (!adapter)
		goto exit;
