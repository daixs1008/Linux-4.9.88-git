...
struct pinconf_ops {
	...
	bool is_generic;
#endif
	int (*pin_config_get) (struct pinctrl_dev *pctldev,   //获得当前配置
			       unsigned pin,
			       unsigned long *config);
	int (*pin_config_set) (struct pinctrl_dev *pctldev,  //设置当前配置
			       unsigned pin,
			       unsigned long *configs,
			       unsigned num_configs);
	int (*pin_config_group_get) (struct pinctrl_dev *pctldev,  //获得某一组引脚的配置
				     unsigned selector,
				     unsigned long *config);
	int (*pin_config_group_set) (struct pinctrl_dev *pctldev,  //设置某一组引脚的配置
				     unsigned selector,
				     unsigned long *configs,
				     unsigned num_configs);
